...
const ProductDetail = () => {
  ...
  const getProductDetail = async () => {
    let url = `https://my-json-server.typicode.com/rok92/hnm-shoppingmall/products/${id}`;
    ...
    <Container>
      <Row>
        ...
        <Col>
          ...
          <div className="product_letter">
            ₩{product?.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
          </div>
          <div className="dropdown_p">
            {product?.choice ? "Conciuos Choice" : ""}
          </div>
          <Dropdown>
